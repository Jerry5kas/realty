...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-lg border-2 overflow-hidden" style="border-color: {{ $theme['primary_color'] }};">
        <div class="px-6 py-4 border-b" style="background-color: {{ $theme['primary_color'] }}; border-color: {{ $theme['secondary_color'] }};">
            <h2 class="text-xl font-semibold text-white">Edit Property Type Information</h2>
        </div>

        <form action="{{ route('property-types.update', $propertyType) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <x-form-select
                label="Category"
                name="category"
                :value="old('category', $propertyType->category)"
                :options="[
            ...
            <x-form-input
                ...
                name="order"
                type="number"
                :value="old('order', $propertyType->order)"
                min="0"
                placeholder="0"
            />
            ...
            <x-form-checkbox
                label="Active"
                name="is_active"
                :checked="old('is_active', $propertyType->is_active)"
            />
