...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-lg border-2 overflow-hidden" style="border-color: {{ $theme['primary_color'] }};">
        <div class="px-6 py-4 border-b" style="background-color: {{ $theme['primary_color'] }}; border-color: {{ $theme['secondary_color'] }};">
            <h2 class="text-xl font-semibold text-white">Edit Media Asset</h2>
        </div>

        <form action="{{ route('media-assets.update', $mediaAsset) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <x-imagekit-uploader
                        name="file_url"
                        :value="old('file_url', $mediaAsset->file_url)"
                        folder="media-assets"
                    />
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <x-form-select
                        label="File Type"
                        name="file_type"
                        :value="old('file_type', $mediaAsset->file_type)"
                        required
                        :options="[
                            'image' => 'Image',
                            'icon' => 'Icon',
                    ...
                    <x-form-select
                        label="Category"
                        name="category"
                        :value="old('category', $mediaAsset->category)"
                        :options="[
                ...
                <x-form-textarea
                    label="Description"
                    name="description"
                    :value="old('description', $mediaAsset->description)"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <x-form-input
                        label="File Size"
                        name="file_size"
                        :value="old('file_size', $mediaAsset->file_size)"
                    ...
                    <x-form-input
                        label="Dimensions"
                        name="dimensions"
                        :value="old('dimensions', $mediaAsset->dimensions)"
                ...
                <div class="border-t pt-6">
                    <x-form-checkbox
                        label="Active"
                        name="is_active"
                        :checked="old('is_active', $mediaAsset->is_active)"
                    />
                </div>
            </div>
