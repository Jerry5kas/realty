...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-lg border-2 overflow-hidden" style="border-color: {{ $theme['primary_color'] }};">
        <div class="px-4 md:px-6 py-4 border-b" style="background-color: {{ $theme['primary_color'] }}; border-color: {{ $theme['secondary_color'] }};">
            <h2 class="text-lg md:text-xl font-semibold" style="color: white;">Add New User</h2>
        </div>

        <form action="{{ route('users.store') }}" method="POST" class="p-4 md:p-6">
            @csrf
            ...
            <div class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-form-input label="Full Name" name="name" :required="true" placeholder="Enter full name" />
                    <x-form-input label="Email" name="email" type="email" :required="true" placeholder="Enter email address" />
                </div>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-form-input label="Password" name="password" type="password" :required="true" placeholder="Enter password (min 8 characters)" />
                    <x-form-input label="Confirm Password" name="password_confirmation" type="password" :required="true" placeholder="Confirm password" />
                </div>

                <x-form-select label="Legacy Role" name="role" :required="true" value="user" :options="[
                    'admin' => 'Admin',
                    'agent' => 'Agent',
                    'user' => 'User'
                ]" />
