...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-lg border-2 overflow-hidden" style="border-color: {{ $theme['primary_color'] }};">
        <div class="px-4 md:px-6 py-4 border-b" style="background-color: {{ $theme['primary_color'] }}; border-color: {{ $theme['secondary_color'] }};">
            <h2 class="text-lg md:text-xl font-semibold" style="color: white;">Edit Role</h2>
        </div>

        <form action="{{ route('roles.update', $role) }}" method="POST" class="p-4 md:p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-form-input label="Role Name" name="name" :required="true" :value="$role->name" placeholder="e.g., Sales Manager" />
                    <x-form-input label="Slug" name="slug" :required="true" :value="$role->slug" placeholder="e.g., sales-manager" />
                </div>
                
                <x-form-textarea label="Description" name="description" rows="2" :value="$role->description" placeholder="Enter role description" />
                
                <x-form-checkbox label="Active" name="is_active" :checked="$role->is_active" />
